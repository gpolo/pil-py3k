...
def _save(im, fp, filename):
    ...
    fp.write(bytes("#define im_width %d\n" % im.size[0], encoding='ascii'))
    fp.write(bytes("#define im_height %d\n" % im.size[1], encoding='ascii'))

    hotspot = im.encoderinfo.get("hotspot")
    if hotspot:
        fp.write(bytes("#define im_x_hot %d\n" % hotspot[0], encoding='ascii'))
        fp.write(bytes("#define im_y_hot %d\n" % hotspot[1], encoding='ascii'))
